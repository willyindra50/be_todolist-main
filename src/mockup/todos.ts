...
const ensurePriority = (t: StoredTodo): Todo => ({
  ...t,
  priority: (t.priority as Todo["priority"]) ?? "MEDIUM",
});

export const getTodos = (): Todo[] => todosMockup.map(ensurePriority);

export const addTodo = (newTodo: NewTodo) => {
  const now = new Date();
  const parsedDate =
    newTodo.date instanceof Date
      ? newTodo.date
      : newTodo.date
...
export const getTodo = (todoId: string) => {
  const t = todosMockup.find((todo) => todo.id === todoId);
  return t ? ensurePriority(t) : undefined;
};
